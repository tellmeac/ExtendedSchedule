...
type DaySchedule struct {
	...
	Lessons []LessonWithContext `json:"lessons"`
}

func (day *DaySchedule) ExcludeLessons(excluded []ExcludeRule) error {
	var ruleMap = make(map[string][]ExcludeRule, len(excluded))
	for _, e := range excluded {
		ruleMap[e.LessonID] = append(ruleMap[e.LessonID], e)
	}

	var filteredLessons = make([]LessonWithContext, 0)
...
func (day DaySchedule) isExcluded(lesson *LessonWithContext, ruleMap map[string][]ExcludeRule) bool {
	matchGroups := func(a []GroupInfo, b []GroupInfo) bool {
		return reflect.DeepEqual(a, b)
	}

	var lessonMatchExcluded = func(excluded *ExcludeRule) bool {
...
func (day *DaySchedule) Join(other DaySchedule) error {
	var appendDistinct = func(others []*LessonWithContext, lesson *LessonWithContext) []*LessonWithContext {
	...
	var maxPosition = 0
	var lessonsByPosition = make(map[int][]*LessonWithContext, 0)
	...
	var joinedLessons = make([]LessonWithContext, 0, maximumLessons)
